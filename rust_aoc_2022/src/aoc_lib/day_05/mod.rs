...
impl Move {
    ...
    pub fn execute_9001_on(&self, towers: &mut Towers) {
        ...
        to_move
            .into_iter()
            .rev()
            .for_each(|e| towers.put(e, self.to));
    }
}
...
impl Towers {
    ...
    pub fn pop(&mut self, tower: usize) -> String {
        self.towers[tower - 1]
            .pop()
            .expect("can't pop off an empty tower")
    }
...
pub fn tower_moves_parse(path: &Path) -> Result<(Towers, Vec<Move>), Box<dyn std::error::Error>> {
