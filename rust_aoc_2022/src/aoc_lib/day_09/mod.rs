...
pub fn make_moves_from_file(
    path: &Path,
    knots: usize,
) -> Result<HeadTailPositionsSeries, Box<dyn std::error::Error>> {
...
impl HeadTailPositions {
    pub fn new() -> Self {
        Self {
            head_pos: (0, 0),
            tail_pos: (0, 0),
            head_pos_set: HashSet::from([(0, 0)]),
            tail_pos_set: HashSet::from([(0, 0)]),
        }
    }
